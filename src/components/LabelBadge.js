...
export default class LabelBadge extends React.Component {
  ...
  render() {
    ...
    const labelClasses = 'bg-light text-muted rounded-start d-inline-flex align-self-center h-100 px-3 py-2';
    ...
    return (
      <span className={`border rounded d-inline-flex flex-row justify-content-between align-items-center ${className}`}>
        ...
        {removable ? <Close className="me-2" onClick={onRemove} /> : null}
      </span>
    );
  }
}
