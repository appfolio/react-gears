...
describe('<BoundFormRow />', () => {
  describe('with context', () => {
    ...
    const component = shallow(<BoundFormRow name="firstName" />, {
      context: {
        value: {
  ...
  describe('with false-like values', () => {
    const component = shallow(<BoundFormRow name="amount" />, {
      context: {
        value: {
          amount: 0,
        },
  ...
  describe('with defaults', () => {
    ...
    const onChangeFactory = () => onChange;
    const component = shallow(<BoundFormRow name="firstName" />, {
      context: { onChange: onChangeFactory },
    });
  ...
  describe('with onChange prop', () => {
    ...
    const onChangeFactory = (name) => (data) => {
      spy(`${name} and ${data}`);
    };
    const onChange = sinon.stub();

    const component = shallow(<BoundFormRow name="firstName" onChange={onChange} />, {
      context: { onChange: onChangeFactory },
    });
