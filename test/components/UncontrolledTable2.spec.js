...
describe('<UncontrolledTable />', () => {
  ...
  it('should render all columns', () => {
    ...
    headers.forEach((th, i) =>
      assert(th.text().indexOf(columns[i].header) > -1)
    );
  });
  ...
  it('should render cell components', () => {
    ...
    classNames.forEach((name) => {
      ...
      assert.equal(
        cells.length,
        rows.length,
        'Column cell not rendered for each row'
      );
    });
  });
  ...
  it('should render correct sort icons when specified', () => {
    ...
    const wrapper = mount(
      <UncontrolledTable
        columns={columns}
        sort={{ column: 'alpha', ascending: true }}
      />
    );
  ...
  it('should render column widths if specified', () => {
    ...
    columns.forEach((column, i) =>
      assert.equal(col.get(i).props.style.width, column.width)
    );
  });
  ...
  it('should render correct align when present', () => {
    ...
    const wrapper = mount(
      <UncontrolledTable columns={columns} rows={[1, 2, 3]} />
    );

    assert.equal(
      wrapper.find('thead th.text-left').length,
      1,
      'thead th.text-left incorrect'
    );
    assert.equal(
      wrapper.find('thead th.text-center').length,
      1,
      'thead th.text-center incorrect'
    );
    assert.equal(
      wrapper.find('thead th.text-right').length,
      1,
      'thead th.text-right incorrect'
    );

    assert.equal(
      wrapper.find('tbody td.text-left').length,
      3,
      'tbody td.text-left incorrect'
    );
    assert.equal(
      wrapper.find('tbody td.text-center').length,
      3,
      'tbody td.text-center incorrect'
    );
    assert.equal(
      wrapper.find('tbody td.text-right').length,
      3,
      'tbody td.text-right incorrect'
    );

    assert.equal(
      wrapper.find('tfoot td.text-left').length,
      1,
      'tfoot td.text-left incorrect'
    );
    assert.equal(
      wrapper.find('tfoot td.text-center').length,
      1,
      'tfoot td.text-center incorrect'
    );
    assert.equal(
      wrapper.find('tfoot td.text-right').length,
      1,
      'tfoot td.text-right incorrect'
    );
  });
  ...
  it('should render correct column classnames when present', () => {
    ...
    assert.equal(
      wrapper.find('thead th.whatever').length,
      1,
      'thead th.whatever incorrect'
    );

    assert.equal(
      wrapper.find('tbody td.whatever').length,
      3,
      'tbody td.whatever incorrect'
    );

    assert.equal(
      wrapper.find('tfoot td.whatever').length,
      1,
      'tfoot td.whatever incorrect'
    );
  });
  ...
  it('should render select column when specified', () => {
    ...
    const rows = ['Alpha', 'Bravo', 'Charlie', 'Delta'];
    const wrapper = mount(
      <UncontrolledTable columns={columns} rows={rows} selectable />
    );
  ...
  it('should show correct rows on page change', () => {
    ...
    assert.deepStrictEqual(table.prop('rows'), [
      'Alpha',
      'Bravo',
      'Charlie',
      'Delta',
  ...
  it('should toggle expanded correctly', () => {
    ...
    sinon.assert.calledOnceWithExactly(onExpandSpy, { name: 'Bravo' });
  ...
  describe('isEqualUsingKeys()', () => {
    ...
    it('should use object comparison if no keys present in arrays', () => {
      ...
      assert.strictEqual(
        wrapper.instance().isEqualUsingKeys(oldArray, newArray),
        false
      );
      oldArray = newArray;
      assert.strictEqual(
  ...
  describe('UNSAFE_componentWillReceiveProps()', () => {
    ...
    it('should not reset state for expanded, page, and selected if a rows key attributes have not changed', () => {
      assert(
        wrapper.props().expanded.length > 0,
        'the expanded props should be non empty'
      );
      assert(
        wrapper.props().selected.length > 0,
        'the selected props should be non empty'
      );
    ...
    it('should reset state for expanded, page, and selected if a rows key attributes have changed', () => {
      ...
      assert.strictEqual(
        wrapper.state().page,
        0,
        'the page state should have been set to 0'
      );
    });
